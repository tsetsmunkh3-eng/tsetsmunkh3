...
export default function GamesArea() {
  ...
  const [activeGame, setActiveGame] = useState<'none' | 'anime' | 'anime-emoji' | 'basketball'>('none');
  ...
  const getActiveQuestionsList = () => {
    return activeGame === 'anime' ? ANIME_TRIVIA_QUESTIONS : questions;
  };

  // Anime Guesser Timer Effect
  useEffect(() => {
    if ((activeGame === 'anime' || activeGame === 'anime-emoji') && gameState === 'playing' && selectedAnswer === null) {
      if (timeLeft > 0) {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {activeGame === 'none' ? (
          <div id="games-selection-grid" className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
        ...
        ) : (activeGame === 'anime' || activeGame === 'anime-emoji') ? (() => {
          const activeList = getActiveQuestionsList();
          const isEmojiMode = activeGame === 'anime-emoji';
          const activeHighScore = isEmojiMode ? emojiHighScore : highScore;
          const gameTitleMN = isEmojiMode ? 'Аниме Эможи Таавар' : 'Аниме Тривиа Асуулт';
          const gameTitleEN = isEmojiMode ? 'Anime Emoji Guesser' : 'Anime Trivia Guesser';
                ...
                <div className="flex items-center justify-between border-b border-zinc-800 pb-4 mb-6">
                  <div className="flex items-center gap-4">
                    <span className={`text-sm font-mono font-bold tracking-wider uppercase ${
                      isEmojiMode ? 'text-yellow-500' : 'text-[#e8702a]'
                    }`}>
                            ...
                            return (
                              <motion.button
                                ...
                                whileHover={selectedAnswer === null ? {
                                  scale: 1.03,
                                  boxShadow: isEmojiMode ? "0 0 15px rgba(234, 179, 8, 0.15)" : "0 0 15px rgba(232, 112, 42, 0.15)",
                                  borderColor: isEmojiMode ? "rgba(234, 179, 8, 0.5)" : "rgba(232, 112, 42, 0.5)",
